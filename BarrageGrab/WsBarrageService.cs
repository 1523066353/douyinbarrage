...
namespace BarrageGrab
{
    ...
    public class WsBarrageService
    {
        ...
        private void Grab_OnGiftMessage(object sender, GiftMessage e)
        {
            ...
            //内部直接获取礼物动态
            TCPServer.NetInit.OnSendMsg((int)EventType.ShowText+ "||" + enty.GiftId.ToString());
        ...
        private void Grab_OnMemberMessage(object sender, Modles.ProtoEntity.MemberMessage e)
        {
            ...
            TCPServer.NetInit.OnSendMsg((int)EventType.ShowUserJion + "||" + e.User.Nickname);
            ...
            var json = JsonConvert.SerializeObject(pack);
            this.Broadcast(json);
            speech.SpeakAsync("欢迎，"+ e.User.Nickname);
        }
        ...
        private void Grab_OnChatMessage(object sender, ChatMessage e)
        {
            ...
            //string das = string.Format("{0}  {1}: {2}",enty.User.GenderToString(),enty.User.Nickname,enty.Content);
            string das = string.Format("{0}||{1}:{2}",(int)EventType.ShowDigio,enty.User.Nickname,enty.Content);
            TCPServer.NetInit.OnSendMsg(das);
        ...
        public void StartListen()
        {
            ...
            console.WriteLine($"{this.socketServer.Location} 抖音unity弹幕服务已启动...", ConsoleColor.Green);
